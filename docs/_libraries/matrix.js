class Matrix {
  constructor(dims) {
    ...
    for (let i = 0; i < dims[1]; i++) this.list[i] = new Vec(dims[0]);
  }
  ...
  static mono(num, dims) {
    ...
    for (let i = 0; i < list.length; i++) list[i] = Vec.mono(num, dims[0]);
  ...
  det() {
    ...
    let product = 1;
    for (let i = 0; i < numRows; i++) product *= calcMat.getNum([i, i]);
    if (odd_swaps) product *= -1;
    return product;
  }
  ...
  inv() {
    ...
    for (let fd = 0; fd < numRows; fd++) {
      ...
      inv.getRow(fd).divNum(pivot);

      this.getRow(fd).divNum(pivot);
      for (let i = fd + 1; i < numRows; i++) {
        ...
        const iSubRow = inv.getRow(fd).copy().mulNum(scaler);

        inv.getRow(i).subVec(iSubRow);
      }
    }
    ...
    for (let fd = 1; fd < numRows; fd++) {
      for (let i = 0; i < fd; i++) {
        ...
        const cSubRow = this.getRow(fd).copy().mulNum(scaler);
        this.getRow(i).subVec(cSubRow);
  ...
  transpose() {
    this.list = this.list[0].list.map((col, i) =>
      Vec.fromList(this.list.map((row) => row.ind(i)))
    );
    return this;
  }
  ...
  mulMat(matrix) {
    ...
    if (num_col != num_row) return null;
  ...
  equ(matrix) {
    ...
    for (const i in thisDim) if (thisDim[i] != matDim[i]) return false;

    for (const i in this.list)
      if (!this.list[i].equ(matrix.list[i])) return false;
  ...
  disp_str() {
    let toReturn = "";
    for (const row of this.list) toReturn += `${row.list}\n`;
    return toReturn;
  }

  toString() {
    return this.disp_str();
  }

  copy() {
    let newList = [...this.list];
    for (const i in newList) newList[i] = newList[i].copy();
